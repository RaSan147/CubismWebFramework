...
> implements ICubismClippingManager {
    ...
    if (usingClipCount > 0) {
      ...
      if (this._clearedFrameBufferFlags.length != this._renderTextureCount) {
        ...
        for (let i = 0; i < this._renderTextureCount; i++) {
          this._clearedFrameBufferFlags.push(false);
        }
      } else {
        // マスクのクリアフラグを毎フレーム開始時に初期化
        for (let i = 0; i < this._renderTextureCount; i++) {
          this._clearedFrameBufferFlags[i] = false;
        }
      }
      // 実際にマスクを生成する
  ...
  protected _clearedFrameBufferFlags: boolean[]; //マスクのクリアフラグの配列
  protected _channelColors: CubismTextureColor[];
  protected _clippingContextListForMask: T_ClippingContext[]; // マスク用クリッピングコンテキストのリスト
  protected _clippingContextListForDraw: T_ClippingContext[]; // 描画用クリッピングコンテキストのリスト
